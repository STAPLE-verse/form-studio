...
function generateDependencyElement(name, dataProps, uiProperties, requiredNames, categoryHash, definitionData, definitionUi, useDefinitionDetails = true // determines whether to use an element's definition details or not.
) {
    let uiProps = {
        ...uiProperties,
    };
...
export function generateElementComponentsFromSchemas(parameters) {
    const { schemaData, uiSchemaData, onChange, definitionData, definitionUi, hideKey, path, cardOpenState, setCardOpenState, allFormInputs, mods, categoryHash, Card, Section, } = parameters;
    ...
    const elementList = elementPropArr.map((elementProp, index) => {
        ...
        if (elementProp.propType === "card") {
            ...
            return (_jsx(Card, { componentProps: Object.assign({
                ...
                }, elementPropArr[index].uiOptions, elementPropArr[index].dataOptions), TypeSpecificParameters: TypeSpecificParameters, onChange: (newCardObj) => {
                    const newElementObjArr = generateElementPropsFromSchemas({
                        schema,
                        uischema,
                        definitionData,
                ...
                }, cardOpen: expanded, setCardOpen: (newState) => setCardOpenState({
                    ...cardOpenState,
                    [elementKey]: newState,
                }), allFormInputs: allFormInputs, mods: mods, addProperties: addProperties }, elementKey));
        }
            ...
            return (_jsx(Section, { schema: elementProp.schema, uischema: elementProp.uischema, onChange: (newSchema, newUiSchema, newRef) => {
                    const newElementObjArr = generateElementPropsFromSchemas({
                        schema,
                        uischema,
                        definitionData,
                ...
                }, name: elementProp.name, required: elementProp.required, path: `${path}_${elementProp.name}`, definitionData: definitionData || {}, definitionUi: definitionUi || {}, hideKey: hideKey, reference: elementProp.$ref, neighborNames: elementProp.neighborNames, dependents: elementProp.dependents, dependent: elementProp.dependent, parent: elementProp.parent, parentProperties: addProperties, cardOpen: expanded, setCardOpen: (newState) => setCardOpenState({
                    ...cardOpenState,
                    [elementKey]: newState,
                }), allFormInputs: allFormInputs, categoryHash: categoryHash, mods: mods }, elementKey));
        }
        else {
            return (_jsx("div", { children: _jsx("h2", { children: " Error parsing element " }) }, elementKey));
        }
    });
    return elementList;
}
...
function propagateElementChange(elementArr, definitionData, definitionUi, categoryHash) {
    ...
    elementArr.forEach((element) => {
        ...
        if (element.propType === "section") {
            ...
            const newUiSchema = Object.assign({ ...element.uischema }, generateSchemaFromElementProps(updatedChildren));
            const newSchema = Object.assign({ ...element.schema }, generateSchemaFromElementProps(updatedChildren));
            const newElement = {
                ...element,
                schema: newSchema,
                uischema: newUiSchema,
...
export function excludeKeys(obj, keys) {
    ...
    return Object.keys(obj).reduce((acc, curr) => (keysHash[curr] ? acc : { ...acc, [curr]: obj[curr] }), {});
}
